...
class Experience(object):
    ...
    def __init__(self, conf):
        ...
        self.priority_size = conf['priority_size'] if 'priority_size' in conf else self.size  # 用于优先级相关操作的容量，默认为整个缓冲区的容量

        self.alpha = conf['alpha'] if 'alpha' in conf else 0.7  # alpha 控制排名对采样概率影响的程度。
        self.beta_zero = conf['beta_zero'] if 'beta_zero' in conf else 0.5  # beta_zero 是初始的校正系数，后面用于计算重要性采样权重。通常公式为
        self.batch_size = conf['batch_size'] if 'batch_size' in conf else 32  # batch_size：每次采样时抽取的经验数，默认为 32。
        self.learn_start = conf['learn_start'] if 'learn_start' in conf else 1000 #learn_start：开始学习（采样）之前需要累积的经验数量，默认为 1000。
        self.total_steps = conf['steps'] if 'steps' in conf else 100000 #total_steps：表示预期的总训练步数（或者用于调节 beta 增长速率的参数），默认为 100000。
        # partition number N, split total size to N part
        self.partition_num = conf['partition_num'] if 'partition_num' in conf else 100 #将整个缓冲区分成多少个部分，常用于构造分段的采样概率分布。

        self.index = 0 #当前插入经验的索引。
        self.record_size = 0 #记录当前保存的经验总数。
        self.isFull = False #布尔标志，指示缓冲区是否已满
